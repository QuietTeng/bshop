...
class Simple extends IController
{
	...
	//重置密码发送短信
	function send_message_mobile_repasswd()
	{
		...
		$userDB = new IModel('member');
		$userRow = $userDB->getObj('user_id='.$user_id);
		if($userRow)
		{
			...
			if($findPasswordDB->add())
			{
				...
				//调用记录
				$data = array('user'=>1,'mobile'=>$mobile,'content'=>$content,'result'=>$result);
				$log_code   = new IModel('log_code');
				$log_code->setData($data);
				$log_code->add();
				if($result == 'success')
				{
    ...
    function favorite_add()
    {
    	...
    	if($goods_id){
    		$ob_name = '商品';
    		$where_field = "rid='".$goods_id."'";
    	}elseif($seller_id){
    		$ob_name = '店铺';
    		$where_field = "seller_id='".$seller_id."'";
    	}

    	if($goods_id == 0&&$seller_id == 0)
    	{
    		$message = $ob_name.'id值不能为空';
    	}
    	...
    	else
    	{
    		...
    		$goodsRow    = $favoriteObj->getObj('user_id = '.$this->user['user_id'].' and '.$where_field);
    		if($goodsRow)
    		{
    			$message = '您已经收藏过此'.$ob_name;
    		}
    		else
    		{
	    		...
	    		if($goods_id){
			    	...
			    	$sellerDB->setData(array("favorite" => "favorite + 1"));
			    	$sellerDB->update("id = ".$seller_id,'favorite');
	    		}
