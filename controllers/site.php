...
class Site extends IController
{
	...
	function index()
	{
		$this->index_slide = apple::go('getBannerList');
		$this->redirect('index');
	}
	...
	function article()
	{
		...
		$catRow = apple::go('getArticleCategoryInfo',$catId);
		$queryArticle = $catRow ? apple::go('getArticleListByCatid',$catRow['id']) : apple::go('getArticleList');
	...
	function article_detail()
	{
		...
		else
		{
			...
			//关联商品
			$this->relationList = apple::go('getArticleGoods',array("#article_id#",$this->article_id));
	...
	function comments_list()
	{
		...
		//查询评价数据
		$this->commentQuery = apple::go('getListByGoods',$id,$point);
		$this->commentCount = Comment_Class::get_comment_info($id);
		$this->goods        = apple::go('getGoodsInfo',array("#id#",$id));
	...
	function comments()
	{
		...
		$this->goods        = apple::go('getGoodsInfo',array("#id#",$result['goods_id']));
	...
	function pic_show()
	{
		...
		$id   = IFilter::act(IReq::get('id'),'int');
		$item = apple::go('getGoodsInfo',array('#id#',$id));
		if(!$item)
		{
			IError::show(403,'商品信息不存在');
		}
		$photo = apple::go('getGoodsPhotoRelationList',array('#id#',$id));
	...
	function help_list()
	{
		...
		if($cat_row)
		{
			$this->helpQuery = apple::go('getHelpListByCatId',$id);
			$this->cat_row   = $cat_row;
		}
		else
		{
			$this->helpQuery = apple::go('getHelpList');
	...
	function groupon()
	{
		...
		if($id)
		{
			$this->regiment_list = apple::go('getRegimentRowById',array('#id#',$id));
			$this->regiment_list = $this->regiment_list ? array($this->regiment_list) : array();
		}
		else
		{
			$this->regiment_list = apple::go('getRegimentList');
		}
		...
		//往期团购
		$this->ever_list = apple::go('getEverRegimentList');
	...
	function brand_zone()
	{
		...
		$brandRow = apple::go('getBrandInfo',$brandId);
		if(!$brandRow)
		{
	...
	{	
		...
		$this->index_slide = apple::go('getBannerList',$seller_id);
		$sellerRow = apple::go('getSellerInfo',$seller_id);
		if(!$sellerRow)
		{
