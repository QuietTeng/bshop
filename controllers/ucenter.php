...
class Ucenter extends IController implements userAuthorization
{
    ...
    {
    	//获取用户基本信息
		$user = apple::go('getMemberInfo',$this->user['user_id']);

		//获取用户各项统计数据
		$statistics = apple::go('getMemberTongJi',$this->user['user_id']);
		...
		$propData= apple::go('getPropTongJi',$propIds);
    ...
    {
        $this->order_status = IReq::get('order_status','post') != null?IFilter::act(IReq::get('order_status','post') ,'int') : -1;
        $this->seller_id = IReq::get('seller_id','post') != null?IFilter::act(IReq::get('seller_id','post') ,'int') : -1;
        $this->initPayment();
        $this->redirect('order');
    }
    /**
     ...
     * @brief 锁定余额
     */
    public function luck_balance()
    {
        $model = new IModel('member');
        $model->setData(array('lock_balance' => "((lock_balance+1)%2)"));
        $model->update("user_id = ".$this->user['user_id'],array('lock_balance'));
    ...
    function password_edit()
    {
    	...
    	else if($mobile_code != ISafe::get("code".$row[0]['mobile']))
    	{
    		$message  = '验证码错误';
    	}
    	else
    	{   
	    	...
	    	if($result)
	    	{
	    		ISafe::set('user_pwd',md5($password));
                ISafe::set("code".$row[0]['mobile'],'');
	    		$message = '密码修改成功';
	    	}
	    	else
	    	{
    ...
    function account_log()
    {
        ...
        if($this->historyTime != null && $this->historyTime != 'default')
        {
            ...
            $this->c_datetime = 'time >= "'.$historyStamp.'" and time < "'.$lastTime.'"';
        }
        else
        {
            $this->c_datetime = 'time >= "'.$lastTime.'"';
        }
    	$user_id   = $this->user['user_id'];
    	$memberObj = new IModel('member');
    	$where     = 'user_id = '.$user_id;
    ...
    {
    	$user_id = $this->user['user_id'];
    	$id      = IReq::get('id')?IReq::get('id'):IReq::get('check');
        $act = 'favorite';
    ...
    function redpacket()
    {
		$member_info = apple::go('getMemberInfo',$this->user['user_id']);
