...
class statistics
{
	...
	public static function gradeSeller($seller_id)
	{
		...
		return isset($dataRow['num']) ? round($dataRow['num']) : 0;//评分标准:乐观平均分
	}
	 ...
	 * @return int
	 */
	public static function countUserComplete($user_id)
	{
		$orderDB = new IModel('order');
		$data    = $orderDB->getObj('user_id = '.$user_id.' and status = 5 and if_del = 0','count(id) as num');
		return $data['num'];
	}
