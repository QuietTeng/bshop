...
class APIUcenter
{
	...
	///用户中心-账户余额
	public function getUcenterAccoutLog($userid,$c_datetime)
	{
	...
	//用户中心-订单列表
	public function getOrderList($userid,$status,$seller_id){
		$page = IReq::get('page') ? IFilter::act(IReq::get('page'),'int') : 1;
		$query = new IQuery('order');
    ...
    //[收藏夹]获取收藏夹数据
	public function getFavorite($userid,$cat = '',$seller_id='')
    {
		//获取收藏夹信息
	    $page   = IReq::get('page') ? IFilter::act(IReq::get('page'),'int') : 1;
		...
		if($seller_id){
			$favoriteObj->join  = "left join seller as go on go.id = f.seller_id";
			$favoriteObj->fields= " f.*,go.true_name,go.paper_img";
			$where .= " and f.rid=0 ";
		}else{
		...
		$favoriteObj->pagesize = 5;
		return $favoriteObj;
    }

    public function getOrderSellerList($user_id){
    	$obj =  new IQuery('order as o');
    	$obj->join  = "join seller as s on s.id = o.seller_id";
    	$obj->where = "o.if_del=0 and user_id=".$user_id;
    	$obj->fields = 'true_name,o.seller_id as id';
    	//$obj->debug = 1;
    	return $obj;
    }
}